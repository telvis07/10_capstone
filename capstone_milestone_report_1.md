...
A suffix tree is a data structure where an interior node represents a "root" character sequence from the training data and child node represents a suffix of that root. For our implementation, each node represents a word and an arc (e.g. search path) represents a bigram, trigram or 4-gram. Each node has an associated count that represents the ngram frequency. We use these counts to predict the next word by calculating the maximimum likelihood estimate (MLE). 
